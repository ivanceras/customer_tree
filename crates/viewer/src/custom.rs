...
async fn search_accounts(
    ...
    log::info!("schema: {:#?}", dataframe.schema());
...
async fn execute_sql(db: CustomDataSource) -> Result<()>{
    // create local execution context
    let ctx = SessionContext::new();
    ...
    let logical_plan = LogicalPlanBuilder::scan_with_filters(
        "accounts",
        provider_as_source(Arc::new(db.clone())),
        None,
        vec![],
    )?
    .build()?;
    ctx.register_table("table1", Arc::new(db.clone()))?;
    let df = ctx.sql("select * from table1").await?;
    let result = df.collect().await?;
    log::info!("table1: {:#?}", result);
...
impl ExecutionPlan for CustomExec {
    ...
    fn execute(
        ...
        id_array.append_value(42);
        account_array.append_value(42_000);

        let mut id_array2 = UInt8Builder::with_capacity(1);
        let mut account_array2 = UInt64Builder::with_capacity(1);
        id_array2.append_value(43);
        account_array2.append_value(43_000);
        ...
        Ok(Box::pin(MemoryStream::try_new(
            ...
            RecordBatch::try_new(
                self.projected_schema.clone(),
                vec![
                    Arc::new(id_array2.finish()),
                    Arc::new(account_array2.finish()),
                ],
            )?
            ],
            self.schema(),
